...
# etcd health, apiserver /healthz, node readiness, ceph health.
#
# All SSH is key-based with strict host-key checking. The path to
# `known_hosts` (generated by `nix run .#k8s-gen-secrets`) is plumbed
# in via `knownHostsPath`; the user SSH private key is read from
# `./secrets/ssh-ed25519` at runtime.
#
{ pkgs, lib, knownHostsPath ? null }:
let
  mainConstants = import ../constants.nix;

  # SSH options shared by every helper below. PubkeyAuthentication=no
  # was a deliberate setting in the source repo (the old flow only had
  # the root password); we flip it to publickey-only.
  sshOpts = lib.concatStringsSep " " ([
    ...
    "-o" "IdentityFile=./secrets/ssh-ed25519"
    "-o" "IdentitiesOnly=yes"
  ] ++ (if knownHostsPath != null
        then [ "-o" "UserKnownHostsFile=${knownHostsPath}" ]
        else []));

  # Bash that bails out with a clear error if the host's SSH inputs
  # weren't generated. Prepended to every helper below.
  guardSshInputs = ''
    if [ ! -f ./secrets/ssh-ed25519 ]; then
      echo "ERROR: ./secrets/ssh-ed25519 missing. Run 'nix run .#k8s-gen-secrets' first." >&2
      exit 1
    fi
    ${if knownHostsPath == null then ''
      echo "ERROR: known_hosts not baked into this app build. Run k8s-gen-secrets, then re-evaluate the flake." >&2
      exit 1
    '' else ""}
  '';
in
{
  ...
  mkCheckCephHealthyScript = ''
    check_ceph_healthy() {
      ...
      # Avoids jq dependency on the host by using sed.
      local health up in
  ...
  mkSshHelper = ''
    ssh_cmd() {
      local host="$1"
      shift
      ssh ${sshOpts} "root@$host" "$@" 2>/dev/null
    }
    ...
    wait_for_ssh() {
      ...
      local elapsed=0
      while [[ $elapsed -lt $timeout ]]; do
        if ssh ${sshOpts} "root@$host" true 2>/dev/null; then
          return 0
        fi
        sleep 1
        elapsed=$((elapsed + 1))
