...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    services.prometheus = {
      ...
      # remoteWrite to the in-cluster observability collector has been
      # removed along with the OTel/ClickStack stack. Phase-2 plan:
      # re-add a remoteWrite block pointing at a Rook MGR-scraped
      # Prometheus or any external long-term store.
    };
